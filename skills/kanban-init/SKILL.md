...
**旧版安装(legacy,机制接管)**:有 manifest/机制件/demos、无 config(= 项目里手工长出的旧看板)。apply = ①生成 config——docs 从旧 gen.mjs 的 REF_DOCS 机械翻译提取,`--brand` 按旧板实况给(要线别 UI 则接管后手工在 config 补 `lanes` 对象,见「线别」节);②settings.json 摘除旧 kanban hook 注册(只认 claude-stop-hook / 「看板提醒」两枚标记,其他 hook 与键一律原样),deny 四条缺则补齐;③旧机制件(gen/守卫/serve)**不删除**——自动落「割接清理」+「backnav 换章」两张 backlog 卡,退役与换章由人在割接后单独 PR。**数据合同:四 manifest 只允许 backlog 追加(卡 + `tiers."0"` 词汇),demos 与其余 manifest 逐字节不动。**`tiers."0"` 只是初始兜底词汇,不是新卡的默认档 —— `card new` 建卡的默认 tier 取现有非 done 卡里出现最多的那档,板上还没有别的 tier 时才会落到它。
...
**卡片可选字段(v0.13.1)**:三类卡都可加 `settleHold`(一句话理由,如 `"这一轮 PR 只落了接口,正主还在下一轮"`)。写了它的卡:不出「PR 已合 · 待收账」/「已收账但 PR 未合」芯片,不进「发布进度」的待收账段,收工守卫不点名,`pr-sync --settle` 把它单列成「已 hold」一行且 `--write` 不碰它;卡头改出一枚安静的灰芯片「暂不收账」,理由挂在 title 上。**「PR 合了」不等于「卡能收」** —— 一张卡跨几轮 PR 是常态,这个字段是让人把那句判断写进数据,而不是每周重新做一遍。要收账时删掉字段即可。只在 `release-manifest.json` 在场时有意义;缺省 = 输出逐字节不变。
...
**卡片可选字段(v0.16.0)**:三类卡都可加 `after`(前置依赖,数组)—— 元素是卡号(`"BL-C74"` / `"D89"`,必须在板上,否则 gen 硬报错)、PR(`"#266"` / `"owner/repo#12"`,语法同 `pr`)或版本 tag(`"v0.0.5"`,语法同 `releases[].tag`)。清除判据全部取自已提交的事实:卡的 status 进终态、PR 在 release-manifest 里 `merged`、tag 出现在 `releases[]`;清除日分别是卡收到终态那天(时间线里那条「【日期】status → …」/「【日期 收账】…」,退而取卡上的 `date`)/ `mergedAt` / `at`,**gen 照旧不读时钟**。PR 与版本还没出现在 manifest 里 = 未清,不报错;自指与环 → CLI 拒写、gen 硬报错并点名环。卡头两枚芯片:等着的出灰芯片「等 N 项」(逐项状态挂 title),全清出「前置已清 · MM-DD」,两枚都只在卡未终态时说话;被 ≥1 张未终态的卡指着、且自身未终态的卡出「被 A · B 等着」(面上最多 3 个 + `+N`,点号跳卡)——陈述「谁的前置里有它」,不承诺「清掉它就解锁谁」。**`config.wip` 的计数随之改口**:只数 `ready` 且前置已清的卡,等前置的另计(「可立即做 12(另 5 等前置)」)。收工守卫多一行非阻断的「前置已清:…」(清除日在 7 天内、卡仍 `ready`,最多 5 张)。与 `blockedOn` 分工:那句自由文本留给「等人 / 等外部」这类没有机器判据的理由,两者可同时存在。**字段驱动、无 config 键;板上一条 `after` 都没有 = 输出逐字节不变。**
...
- **`lazyTabs` 开着时验收自成一个 part**(`parts/acceptance.html`,v0.15.0):清单正文与烤入数据都随它走,壳里只留骨架与
  初始化函数;`#acc-<PR>` / `#acc-<清单串>` 深链先取 part 再定位。卡头「验收中 · n/N」的分子要靠这份数据,所以别的 pane
  注入进来时只要带着 `[data-acc]`,运行期会顺手把验收那份先取回来 —— 分子不会停在烤入的 0/N。
- **字节冻结**:不配或 `false` 时输出逐字节不变。

## 验收反馈共享(acceptanceFeedback,v0.17.0:opt-in,需先开 acceptanceTab;v0.17.1「判定即勾选」)

`config.acceptanceFeedback` 缺省关;设 `true` 后验收行左栏的勾选框换成 `✓ / ✕` 两枚判定钮:**点一下即写**,
乐观上色(失败回滚并在行下出一行原因),点已选的那枚 = 撤回(追加 `verdict:"none"`,400ms 内的第二下不算数),
`✓ ↔ ✕` 直接互换。✓ 变淡、✕ 左缘红边;进度与分组计数改口「已判 N / M · 其中 K 条不对」,**全按我的判定算**。
行右下角常驻「＋ 备注 / 图」(点开一行输入框并聚焦,回车或失焦即存,空着不记;判 ✕ 自动展开并聚焦),
贴图/选图上传成功即进账。行末摘要 chip 只在别人说过话时出现(`乙 ✕ · 1 图`);点开的时间线列该条目**全部**记录
(含我自己的),一行一条 = 姓名首字圆点 · ✓/✕ · 时间 · 备注 · 缩略图,撤回记录淡色,缩略图点开就地放大。
...
- **真源是 `app/kanban/acceptance-feedback.jsonl`**(一行一条,进 git):由 `serve.py` 的两个只追加的 POST 口写 ——
  ...
  (原始图片体)。O_APPEND 单次 write + fsync,两人同时点也不会互相截断;不提供删除,改错就再追加一条。
- **页面轮询**:验收 tab 可见期间每 20 s、窗口回到前台、切进这个 tab 时各拉一次;失败静默重试,不弹窗。
  合并口径:同一 (who, 条目) 取最新 `verdict`,备注与图累积;坏行跳过并计数。
- **身份**:第一次判定 / 写备注 / 贴图时问一次「我是」(1–20 字),存这台浏览器的 `<brand>_acc_who`;tab 顶部一枚
...
- **「复制结果」**(0.17.0 的「复制勾选结果」改名)读我的判定,输出 `{"checked":[通过],"bad":[不对],"at":"…"}` ——
  `checked` 键保持原形,贴回清单 `result` 的老路不断;清单 `result.checked` 里的条目在行上渲成只读的「已收」灰标,
  不冒充谁的判定、也不计入进度。**0.17.0 的本机勾选键 `<brand>_acc_<pr>_r<rev>` 读到即忽略、不再写、不迁移**
  (那是私有痕迹,转成判定等于替人署名发布)。
- **无写口时降级**(serve.py 老于 v0.17.0 / 纯静态服):控件形状不变,判定存本机 `<brand>_acc_v_<pr>_r<rev>`,
  tab 顶一行灰字说清「判定只存在这台浏览器里」;每次开页仍试探一次,写口回来了自己接上。
- **截图**:客户端 canvas 压到长边 ≤ 1280 / JPEG 0.8 再传;服务端限 2 MB、校验 JPEG/PNG 魔数,
  **文件名只由服务端拼**(`acc-<PR>-<条目>-<UTC 时刻>.jpg`),不接受客户端文件名。落 `shots/`,不进截图廊。
- **截图默认不进 git**:请在 `.gitignore` 加一行 `app/kanban/shots/acc-*` —— init 只会提醒这一行,**不替你改**
...
- **宿主的 `serve.py` 得是 v0.17.0 那版**(撤回要 v0.17.1 的 `ddd-serve v3`):它是 init 种入件,已在场就不覆盖(可能被改过)。落后时 `init --plan/--apply`
  会说一行,升级 = 用 plugin `templates/serve.py` 覆盖 + 重启 serve。
- **名册与口令(v0.17.6,可选)**:把这个键写成 `{ "pin": "1111" }`(4 位数字字符串,别的形状 gen 与 serve 都硬报错)
  就开名册 —— `app/kanban/acceptance-roster.json`(`{"names":[…]}`,进 git,只加不删;要删名就改文件提交)。
  署名 / 换人时页面先拉一次名册:**在册的名字照旧不问口令**,新名字在同一行多出一格 4 位口令(占位「新名字要口令」),
  回车走 `POST /api/acceptance/who {name,pin}` —— 对了名字进名册并接着把刚点的那一下补记,错了在框内右缘出一行灰字
  「口令不对」且不关框,Esc 取消整次署名。**一个框都不弹**。配了口令之后 `mark` / `shot` 只收名册里的名字(否则 403)。
  口令**不烤进产物**,校验全在服务端(错口令 `sleep 1s` 再 403:不锁不计数,只防手滑连点);要 `ddd-serve v4`。
  名册文件不必先建:配了口令时 `GET /acceptance-roster.json` 由 serve 代答(没建 / 读坏都答空名册),第一个人照样进得去。
  写成 `true` = 没有名册这回事,产物与三条写口与 0.17.5 逐响应零差异(who 那条路答 501);开着反馈没配口令时 `init --plan/--apply` 各说一行。
- **字节冻结**:不配或 `false` 时产物逐字节不变,两条写口 404;只开 `acceptanceFeedback` 不开 `acceptanceTab` =
  一行警告 + 按关处理。写成 `true`(没配口令)时,名册那套一个字节都不烤进产物。
...
- **两个视图切换**:表格(号 / 标题 / 段 / 状态·日期 / 关联卡 / 分支 / 验收进度;表头可按日期与号排序,搜索,已发按版本折叠、
  ...
  带内 PR 贪心打包进最多 6 条泳道,当天开当天合的按号横排,展开高度因此有上界。轴是非线性的:安静的日子窄,
  一天挤了很多个 PR 就把那一天加宽 —— 横轴是那张老图唯一没用起来的一维。繁忙度只定相对宽窄,轴整体按面板实宽拉满
  (短窗口 = 放大,不是把轴截短;自然宽超过面板才横向滚)。放大之后一天有多宽再定字形(v0.15.11,三档由同一个
  「一天多少 px」驱动):< 40px 与从前逐字节相同;≥ 40px 方块随日宽长大、横杠退成两端实心 + 细线;≥ 120px 每个 PR
  一枚带号芯片落在锚点日那一格,跨度退成一条 whisker,一格放不下的收进一枚 `+N`。窗口芯片五档「近 30 天 / 近 2 周 / 本周 / 当日 / 全时段」,
  默认近 30 天,本周从本地周一起算,窗口全在浏览器按本地时钟算(gen 不烤日期),不记进 localStorage;release 是竖线)。
  默认表格,选择记在浏览器里。每行有 `id="pr-<号>"` 锚,`#pr-230` 深链直达(深链一律落到表格)。
- **时间线上悬停出卡**(v0.15.2):当天开当天合的 PR 在常规窗口下只有几个像素宽,连号都写不下(放大到芯片档才写得上)。悬停(150ms)或用键盘聚焦任一条 / 方块 / 芯片,
  旁边浮出一张自绘卡:`PR #N` + 完整标题、状态·日期、分支、关联卡芯片(点了切 pane)、验收 n/N —— 全部从表格那一行现取,
  不烤第二份;原生 `title` 一并撤掉(它截断、要等,还会叠成两层)。折叠的版本带上一段代表一天,悬停列出那天的 PR
  (最多 10 条 + 「还有 N 个」),点带头照旧是展开。Esc 收起,触摸屏第一下出卡、第二下才走链接。
- **数据由脚本写,不由 gen 取**:
...
- **`lazyTabs` 开着时发布进度自成一个 part**(`parts/release.html`,v0.15.0):表格行与两个视图共用的那几份表都随它走
  (时间线的几何算子是代码,照旧留在壳里);`#pr-<号>` 深链先取 part 再落到表格。
...
- **长字段折叠**:超过 400 字的字段烤成两份 —— 第一段(按段落边界截)作预览 + 灰字「展开全文 · N 字」的钮
  (N = 字段全长),点开是全文,再点收起,**不记忆折叠状态**。预览段本身也按高度收(与下面那套同一个
  `3.3em`,v0.15.17):400 字在窄列里仍是十几行,字数管不住卡高。没超阈值的、以及整段没有空行可切的,
  照旧走既有的按高度折叠;两套折叠不叠加。按高度折叠在**卡展开的那一下**量高(v0.15.18:行卡的正文收着时
  `display:none`,量不到;此前它对行卡是空转的),超 3.3 行就收成两行 +「展开 ▾」;开着 `richText` 时
  决策卡的 `source` 徽章也在这条路上。
...
- **决策卡的 `source`** 从此渲染成与 backlog 同款的小徽章(此前是有数据没渲染的死数据)。这一条同样在门控内,
  否则存量板一升级就多出一行。
- **守卫**:开着这个键时,某长文本字段超过 800 字而卡上没有 `detail` → 已提交的老卡出一条非阻断 notice,只一行
  (张数 + 最长的那张);**刚立的卡**(卡文件还没进 `HEAD`,未拆卡的板退「`date` 是今天」)则阻断收工,点名卡号 /
  字段 / 字数并给出 `card set <卡号> detail` 那条命令(v0.16.2)。
  终态卡(`done` / `live` / `closed`)两档都不点 —— 收了的卡不会再改写,点它只会让这条通知永远缩不掉。
...
`config.backlogSort` 缺省关;设 `true` 后 Backlog 工具条里那只排序**下拉**换成一排分段钮,前面一枚安静的
「排序」小灰字。**排序一直都有,只是没人找得到**(它夹在筛选下拉与搜索框中间),而且缺一把最常想用的尺 ——
「这张卡最近动过没有」。

- **四档**:`最近立卡`(建卡日新→旧)· `最近更新`(卡文件最后改动日,新→旧)· `最早立卡` · `按编号`。
  第二档是新的,后两档就是原下拉剩下的两项 —— 换控件不丢功能。
- **默认档 = `最近立卡` = 原样**:板面烤入顺序本来就是建卡日新→旧,所以「板面顺序」与「按建卡日排」
  不该占两颗钮(v0.15.12 曾各占一颗,只差同日的编号方向 —— 点着像坏的,v0.15.13 并成一颗)。
  开着这个键、没点过的板与关着时看起来一模一样。
- **更新日期**是 `cardsDir` 那条既有的每卡更新日(一条 `git log` 批量取,取不到退 mtime),
  烤成 `data-udate`;`data-ord` 是卡在本分区里的位次。**没配 `cardsDir` 的板上更新日 = 建卡日**,
  分段照出,只是两把尺量的是同一个数。
- **搬节点,不重渲**:分区顺序永不变,只在每个分区的 `.cards` 里重新 append。所以筛掉的卡还是筛掉的、
  线别分段照点(BL-C105 那条委托没断)、WIP 计数走的还是每次筛选都会跑的那一趟 `setLine`。
  `最近立卡` 按 `data-ord` 原样还原。
- **记在浏览器里**:`<brand>_bl_sort`,pane 注入时读回(懒加载那条路也算),存了不认识的值就退默认档
  —— v0.15.12 存下的 `cdate-desc` 正好走这条路回到「最近立卡」,不用迁移。
- **归档 pane 这一版不给控件**:同一张卡模板的两枚属性它也带着,将来要加只是接一根线。
- **字节冻结**:不配或 `false` 时输出逐字节不变。
...
- **`order` 字段**:拆分时写入的原数组下标。数组顺序决定截图廊组序与深链表键序(Backlog / 决策两个
  pane 面上的顺序另按卡日期新→旧排,不看它),所以得记下来;gen 按 `order` 再按 id 排,**排完就把它删掉**,卡对象与拆分前逐字段相同。
...
- **写之前先验**:status ∈ 该类卡的 statuses、date 形如 `YYYY-MM-DD`、`pr` 形如 `12` / `"#12"` / `"owner/repo#12"`、`line` ∈ `config.lanes.ids`、`session` ∈ `config.sessionTags`、`after` 里的卡号在板上且不自指不成环。不认识的字段只警告不拒(板上的字段一直在长),`order` 一律不许改 —— 它是拆卡时记下的原数组下标(面上 Backlog / 决策按卡日期新→旧排,不看它)。
