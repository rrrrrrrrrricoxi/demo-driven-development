...
## Backlog 排序分段(backlogSort,v0.15.12:opt-in)

`config.backlogSort` 缺省关;设 `true` 后 Backlog 工具条里那只排序**下拉**换成一排分段钮,前面一枚安静的
「排序」小灰字。**排序一直都有,只是没人找得到**(它夹在筛选下拉与搜索框中间),而且缺一把最常想用的尺 ——
「这张卡最近动过没有」。

- **五档**:`默认`(板面烤入顺序)· `最近更新`(卡文件最后改动日,新→旧)· `最近立卡`(建卡日,新→旧)·
  `最早立卡` · `按编号`。中间那档是新的,后两档就是原下拉剩下的两项 —— 换控件不丢功能。
- **默认档 = 原样**:开着这个键、没点过的板与关着时看起来一模一样。
