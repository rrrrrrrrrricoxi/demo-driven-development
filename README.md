...
Categories at zero do not appear, and if all eight are zero the whole block is gone. The order is fixed, and each line names up to three cards — the rest becomes "and N in all" — because a count alone cannot tell you whether any of it is yours. 0.17.5 gave only a label and a count per category; the labels were jargon and nothing was named, so the only way to act on it was to ask. What a line never carries is a command, a path, or an explanation of how to deal with it: that is what `node <plugin>/scripts/ddd.mjs audit` is for, which prints all three levels in full, and `--session <session tag>` narrows the card-scoped ones to one line of work (`--line` is accepted too, as a synonym). Where `ddd.mjs` actually lives is said once, below and in the board section of your `CLAUDE.md` — the block prints on every Stop, and an absolute install path costs four lines on a phone. Guard and `audit` share one implementation (`scripts/audits.mjs`), so the two can never disagree.
