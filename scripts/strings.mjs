...
const zh = {
  ...
  // v0.16.2:上面那条是「本机也没有更新的版本」时的兜底;本机已经装了更新的版本时,守卫直接把这一轮
  // 交给它跑,不再要求人重启 session。说一行是因为下面那段输出来自另一个版本,不说人会以为守卫串味了。
  hookForwarded: (to, mine, stamp) =>
    `看板守卫已转发到已安装的 v${to}(本 session 绑的是 v${mine},看板产物已是 v${stamp})—— 下面这段是新版守卫的输出,不必重启 session。`,
  noSelfVersion:
...
const en = {
  ...
  hookForwarded: (to, mine, stamp) =>
    `Kanban guard forwarded to the installed v${to} (this session is bound to v${mine}; the board's output is already v${stamp}) — everything below comes from that newer guard, so no session restart is needed.`,
  noSelfVersion:
