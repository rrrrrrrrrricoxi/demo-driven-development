...
const zh = {
  ...
  // v0.17.6:缩成一行 —— 这句的用处是「下面那段不是串味」,三个版本号里只有两个有人看。
  hookForwarded: (to, mine) =>
    `守卫已转发到 v${to}(本 session 绑 v${mine};不必重启)`,
  noSelfVersion:
...
const en = {
  ...
  hookForwarded: (to, mine) =>
    `Guard forwarded to v${to} (this session is bound to v${mine}; no restart needed).`,
  noSelfVersion:
